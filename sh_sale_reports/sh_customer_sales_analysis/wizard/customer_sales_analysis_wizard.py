...
class SalesAnalysisWizard(models.TransientModel):
    ...
    sh_status = fields.Selection([('all', 'All'), ('draft', 'Draft'), ('sent', 'Quotation Sent'), (
        'sale', 'Sales Order'), ('done', 'Locked')], string="Status", default='all')
    ...
    def print_report(self):
        ''' Redirect to report action function'''
        datas = self.read()[0]
        return self.env.ref('sh_sale_reports.sh_cus_sales_analysis_action').report_action([], data=datas)

    def display_report(self):
        datas = self.read()[0]
        report = self.env['report.sh_sale_reports.sh_cus_sale_analysis_doc']
        data_values = report._get_report_values(
            docids=None, data=datas)
        data_values_by_order = data_values.get('order_dic_by_orders')
        data_values_by_products = data_values.get('order_dic_by_products')

        # Order
        if self.report_by == "order":
            self.env['sh.customer.sales.analysis.order'].search([]).unlink()
            if data_values_by_order:
                for customer in data_values_by_order:
                    for order in data_values_by_order[customer]:
                        self.env['sh.customer.sales.analysis.order'].create({
                            'sh_partner_id': order['partner_id'],
                            'name': order['order_number'],
                            'order_date': order['order_date'],
                            'user_id': order['salespersion_id'],
                ...
                for key in order_dic_by_orders.keys():
                    ...
                    for rec in order_dic_by_orders[key]:
                        ...
                        worksheet.write(row, 3, str(rec.get(
                            'sale_currency_symbol'))+str("{:.2f}".format(rec.get('sale_amount'))), center)
                        worksheet.write(row, 4, str(rec.get(
                            'sale_currency_symbol')) + str("{:.2f}".format(rec.get('paid_amount'))), center)
                        worksheet.write(row, 5, str(rec.get(
                            'sale_currency_symbol')) + str("{:.2f}".format(rec.get('balance_amount'))), center)
