...
class ShPaymentReportWizard(models.TransientModel):
    ...
    def print_report(self):
        datas = self.read()[0]
        return self.env.ref('sh_sale_reports.sh_payment_report_action').report_action([], data=datas)

    def display_report(self):
        datas = self.read()[0]
        report = self.env['report.sh_sale_reports.sh_payment_report_doc']
    ...
    def print_xls_report(self):
        ...
        worksheet.write_merge(
            0, 1, 0, 6, 'Invoice Payment Report', heading_format)
        ...
        final_total_col_list = [journal.name for journal in search_journals]
                        ...
                        for partial in partials:
                            ...
                            if invoice.move_type == "out_invoice":
                                if invoice_pay_dic.get(invoice.name, False):
                                    ...
                                    invoice_pay_dic.update({invoice.name: {
                                        counterpart_line.payment_id.journal_id.name: payment_amount,
                                        "Total": payment_amount,
                                        "Invoice": invoice.name,
                                        "Customer": invoice.partner_id.name,
                                        "Invoice Date": str(invoice.invoice_date),
                                        "Salesperson": invoice.invoice_user_id.name if invoice.invoice_user_id else "", "style": ''}})
                            if invoice.move_type == "out_refund":
                                ...
                                if invoice_pay_dic.get(invoice.name, False):
                                    ...
                                    invoice_pay_dic.update({invoice.name: {
                                        counterpart_line.payment_id.journal_id.name: -1 * (payment_amount),
                                        "Total": -1 * (payment_amount),
                                        "Invoice": invoice.name,
                                        "Customer": invoice.partner_id.name,
                                        "Invoice Date": str(invoice.invoice_date),
                                        "Salesperson": invoice.invoice_user_id.name if invoice.invoice_user_id else "", "style": 'font:color red'}})
            final_list = [value for value in invoice_pay_dic.values()]

            total_journal_amount = {}
            for value in invoice_pay_dic.values():
        ...
        for user in user_data_dic:
            ...
            row = row + 2
            worksheet.write_merge(
                row, row, 0, 6, "Sales Person: " + user, bold_center)
            row = row + 2
            col = 0
