...
class PurchaseOrderReport(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        def collect_raw_materials(production):
            ...
            for move in production.move_raw_ids.filtered(lambda m: m.state != 'cancel' and m.product_id.purchase_ok and not m.product_id.sale_ok):
                ...
                if product_name:
                    key = (product_name, product_code)
                    unique_products.add(key)
                    weekly_sales[(year, week_number)][key] += move.product_uom_qty
        ...
        for product in products:
            ...
            product_stock_status[(product.name, product.default_code or '')] = {
        ...
        sorted_products = sorted(unique_products, key=lambda x: x[0])  # sort by product name
